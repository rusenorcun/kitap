...
public class FreshPrincipalFilter extends OncePerRequestFilter {
    ...
    /**
     * Statik dosyalar (stil, betik, kapak) herkese açıktır ve yetki kararı gerektirmez. Bir
     * sayfa onlarca kapak ve betik çektiği için oturumlu her ziyarette bunlar da ayrı ayrı
     * veritabanı sorgusu üretiyordu.
     */
    @Override
    protected boolean shouldNotFilter(HttpServletRequest request) {
        String yol = request.getRequestURI().substring(request.getContextPath().length());
        return yol.startsWith("/css/") || yol.startsWith("/js/") || yol.startsWith("/webjars/")
                || yol.startsWith("/uploads/covers/") || yol.startsWith("/favicon.");
    }
