...
public class ApiExceptionHandler {
    ...
    public ResponseEntity<ApiError> handleDateTime(java.time.DateTimeException ex) {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ApiError("Geçersiz tarih formatı. ISO-8601 formatı bekleniyor."));
    }
    ...
    public ResponseEntity<ApiError> handleMessageNotReadable(org.springframework.http.converter.HttpMessageNotReadableException ex) {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ApiError("Geçersiz istek gövdesi veya JSON formatı."));
    }
    ...
    public ResponseEntity<ApiError> handleTypeMismatch(org.springframework.web.method.annotation.MethodArgumentTypeMismatchException ex) {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ApiError("Geçersiz parametre değeri: " + ex.getName()));
    }

    @ExceptionHandler(Exception.class)
    public ResponseEntity<ApiError> handleGeneral(Exception ex) {
        // Spring MVC'nin istemci hataları (eksik parametre/parça, desteklenmeyen tür vb.) kendi
        // durum kodunu taşır. Bunlar 500 + yığın izli ERROR olarak değil, 4xx olarak döner.
