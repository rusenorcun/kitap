...
public interface BookRequestRepository extends JpaRepository<BookRequest, Long> {
    ...
    @Query(value = """
           ...
           left join fetch r.fulfilledBy
           left join fetch r.meeting.point
           where r.status = app.kitapla.domain.RequestStatus.OPEN and st.blocked = false
             and (lower(b.title) like :desen or lower(b.author) like :desen)
           order by r.createdAt desc, r.id desc
           """,
           countQuery = """
           select count(r) from BookRequest r join r.book b
           where r.status = app.kitapla.domain.RequestStatus.OPEN and r.student.blocked = false
             and (lower(b.title) like :desen or lower(b.author) like :desen)
           """)
