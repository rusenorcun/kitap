...
public interface SwapBookRepository extends JpaRepository<SwapBook, Long> {
    ...
    @Query(value = """
           ...
           join fetch s.book b
           join fetch s.user u
           where s.status = app.kitapla.domain.SwapBookStatus.OPEN and u.id <> :benimId and u.blocked = false
             and (lower(b.title) like :desen or lower(b.author) like :desen)
           order by s.createdAt desc, s.id desc
           """,
           countQuery = """
           select count(s) from SwapBook s join s.book b
           where s.status = app.kitapla.domain.SwapBookStatus.OPEN and s.user.id <> :benimId and s.user.blocked = false
             and (lower(b.title) like :desen or lower(b.author) like :desen)
           """)
