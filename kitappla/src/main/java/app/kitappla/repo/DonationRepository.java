...
import app.kitappla.domain.Donation;
import app.kitappla.domain.DonationStatus;
...
public interface DonationRepository extends JpaRepository<Donation, Long> {
    ...
    @Query(value = """
           ...
           left join fetch d.preferredPoint
           where d.status = app.kitappla.domain.DonationStatus.OPEN and dn.blocked = false
             ...
             and (:yalnizcaKalan = false or d.quantity > (
                    ...
                    where c.donation = d
                      and c.status not in (app.kitappla.domain.ClaimStatus.NO_SHOW, app.kitappla.domain.ClaimStatus.CANCELLED)))
           order by d.createdAt desc, d.id desc
           """,
           countQuery = """
           select count(d) from Donation d join d.book b
           where d.status = app.kitappla.domain.DonationStatus.OPEN and d.donor.blocked = false
             and (:tumSeviyeler = true or d.targetLevel in :seviyeler)
             and (lower(b.title) like :desen or lower(b.author) like :desen)
             and (:yalnizcaKalan = false or d.quantity > (
                    select count(c) from Claim c
