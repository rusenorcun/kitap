...
public interface ConversationRepository extends JpaRepository<Conversation, Long> {
    ...
    @Query("""
           ...
           join fetch c.userA
           join fetch c.userB
           where (c.kind not in (app.kitappla.domain.ConversationKind.REPORT, app.kitappla.domain.ConversationKind.SUPPORT)
                   and (c.userA = :user or c.userB = :user))
               or (c.kind = app.kitappla.domain.ConversationKind.REPORT and exists (
               ...
               or (c.kind = app.kitappla.domain.ConversationKind.SUPPORT and (c.userA = :user or exists (
                       select u.id from User u where u = :user and u.admin = true)))
    ...
    @Query("""
           select count(c) from Conversation c
           where (c.kind = app.kitappla.domain.ConversationKind.REPORT
              ...
              or (c.kind = app.kitappla.domain.ConversationKind.SUPPORT
              and exists (select m.id from Message m where m.conversation = c and m.sender <> :me
