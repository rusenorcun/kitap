...
import app.kitappla.domain.ReportStatus;
...
public interface ReportRepository extends JpaRepository<Report, Long> {
    ...
    @Modifying
    @Query("update Report r set r.reportedUser = null where r.reportedUser = :user")
    void nullifyReportedUser(@Param("user") User user);

    @Modifying
    @Query("update Report r set r.reviewedBy = null where r.reviewedBy = :user")
    void nullifyReviewedBy(@Param("user") User user);
}
