...
@Component
@Order(1)   // SemaOnarim(0) sonrasi, DemoSenaryosu(2) oncesi
public class DataSeeder implements CommandLineRunner {
    ...
    @Value("${kitappla.admin.email}") private String adminEmail;
    @Value("${kitappla.admin.password}") private String adminPassword;
    @Value("${kitappla.admin.name}") private String adminName;
    @Value("${kitappla.upload-dir}") private String uploadDir;

    /** Örnek üyeler, kitaplar ve teslim noktaları oluşturulsun mu? */
    @Value("${kitappla.seed.demo:true}") private boolean demoVerisi;
    ...
    /** Yerel denemenin varsayılanı; üretimde bu şifreyle açılmak yasak. */
    ...
    public void run(String... args) {
        ...
        String resolvedAdminPassword = (adminPassword == null || adminPassword.isBlank())
                ? VARSAYILAN_SIFRE : adminPassword.trim();

        // Üretimde KITAPPLA_ADMIN_PASSWORD tanımsızsa sessizce varsayılana düşülüyordu: yönetici
        // hesabı her açılışta bilinen bir şifreye geri dönerdi. Böyle bir kurulum açılmasın.
        if (VARSAYILAN_SIFRE.equals(resolvedAdminPassword)) {
            if (environment.acceptsProfiles(org.springframework.core.env.Profiles.of("prod"))) {
                throw new IllegalStateException(
                        "KITAPPLA_ADMIN_PASSWORD tanımlı değil. Üretimde yönetici hesabı varsayılan "
                                + "şifreyle (" + VARSAYILAN_SIFRE + ") açılamaz; ortam değişkenini ayarla.");
            }
            log.warn("Yönetici hesabı varsayılan şifreyle çalışıyor ({}). Yerel deneme dışında "
                    + "KITAPPLA_ADMIN_PASSWORD ortam değişkenini ayarla.", normalizedAdminEmail);
        }
        ...
        if (!demoVerisi) {
            log.info("Örnek veri kapalı (kitappla.seed.demo=false); yalnızca yönetici hesabı hazır.");
            return;
        }
