...
public class SemaOnarim implements CommandLineRunner {
    ...
    private static final String DOMAIN_PAKETI = "app.kitappla.domain";

    private static final String ESKI_KISIT = "uq_conversations_kind_ref";
    private static final String YENI_KISIT = "uq_conversations_active_ref";
    ...
    public SemaOnarim(JdbcTemplate jdbc, org.springframework.core.env.Environment environment) {
        this.jdbc = jdbc;
    ...
    public void run(String... args) {
        conversationDenemesiGecerliKil();
        jetonTuruKisitiniKaldir();
        int toplam = 0;
    ...
    void conversationDenemesiGecerliKil() {
        if (environment.acceptsProfiles(org.springframework.core.env.Profiles.of("prod"))
                ...
                || !"update".equalsIgnoreCase(environment.getProperty("spring.jpa.hibernate.ddl-auto"))) return;
        try {
            Boolean h2 = jdbc.execute((org.springframework.jdbc.core.ConnectionCallback<Boolean>)
                    connection -> "H2".equals(connection.getMetaData().getDatabaseProductName()));
            if (!Boolean.TRUE.equals(h2)) return;
            jdbc.execute("alter table conversations add column if not exists active_key bigint default 0 not null");
            jdbc.update("update conversations set active_key = 0 where active_key is null");
            jdbc.execute("alter table conversations alter column active_key set default 0");
            jdbc.execute("alter table conversations alter column active_key set not null");
            jdbc.update("""
                    update conversations set active_key = id
                     where kind = 'REQUEST' and active_key = 0
                       and not exists (
                    ...
                    """);
            if (!kisitVar(YENI_KISIT)) {
                jdbc.execute("alter table conversations add constraint " + YENI_KISIT
                        + " unique (kind, ref_id, active_key)");
            }
            List<String> columns = jdbc.queryForList("""
                    select column_name from information_schema.key_column_usage
                     where constraint_schema = current_schema and table_name = 'CONVERSATIONS'
                       and constraint_name = ? order by ordinal_position
                    """, String.class, YENI_KISIT.toUpperCase(Locale.ROOT));
            if (columns.size() != 3 || !new java.util.HashSet<>(columns)
                    .equals(java.util.Set.of("KIND", "REF_ID", "ACTIVE_KEY")))
                throw new IllegalStateException("Sohbet tekil kısıtı beklenen sütunlarla eşleşmiyor: " + columns);
            if (kisitVar(ESKI_KISIT))
                jdbc.execute("alter table conversations drop constraint " + ESKI_KISIT);
        } catch (Exception ex) {
            throw new IllegalStateException("Sohbet şeması yükseltilemedi; tohumlama durduruldu.", ex);
        }
    }

    /**
     * Flyway'siz ({@code ddl-auto=update}) kurulmuş PostgreSQL şemasında Hibernate, jeton türü
     * sütununa o günkü enum değerleriyle bir kısıt koymuştur ve {@code update} bu kısıtı hiç
     * güncellemez: yeni {@code ACCOUNT_VERIFY} jetonu yazılamaz, kayıt 500 verirdi. Flyway
     * açıkken aynı işi V9 yapar.
     */
    void jetonTuruKisitiniKaldir() {
        if (environment.getProperty("spring.flyway.enabled", Boolean.class, true)) return;
        try {
            jdbc.execute("alter table auth_tokens drop constraint if exists auth_tokens_type_check");
        } catch (Exception ex) {
            log.debug("Jeton türü kısıtı kaldırılamadı: {}", ex.getMessage());
        }
    }
