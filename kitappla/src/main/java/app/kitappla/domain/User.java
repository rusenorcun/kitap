...
public class User {
    ...
    /**
     * Giriş e-postası bağlantıyla doğrulandı mı. Doğrulanmamış hesap oturum açabilir ama işlem
     * yapamaz (bkz. EpostaDogrulamaFiltresi). Varsayılan true: bu alan eklenmeden önce açılmış
     * hesaplar doğrulanmış sayılır; yeni kayıtta {@code UserService.register} false yapar.
     */
    @Column(nullable = false)
    @ColumnDefault("true")
    ...
    @Transient
    private boolean studentVerificationSent;

    /** Bu istekte hesap doğrulama bağlantısı teslim sırasına alındı mı (kayıt mesajı için). */
    @Transient
