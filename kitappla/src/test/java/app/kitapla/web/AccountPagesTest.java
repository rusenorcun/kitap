...
class AccountPagesTest {
    ...
    @Test
    void reddedilenOgrenciPanodaYenidenBasvuruBaglantisiGorur() throws Exception {
        User u = mk("reddedilen", "Adres");
        u.setStudentStatus(StudentStatus.REJECTED);
        users.save(u);
        mvc.perform(get("/panom").with(user(as(u))))
                .andExpect(content().string(containsString("başvurun onaylanmadı")))
                .andExpect(content().string(containsString("href=\"/profil/ogrenci\"")));
    }
    ...
    void sifreBasariylaDegisir() throws Exception {
        ...
        mvc.perform(post("/profil/sifre").with(user(as(u))).with(csrf())
                        ...
                        .param("newPassword", "yenisifre1")
                        .param("confirmPassword", "yenisifre1"))
                .andExpect(redirectedUrl("/login?sifre-degisti"));
