...
class StudentEmailVerificationTest {
    ...
    @Autowired app.kitapla.repo.NotificationRepository notifications;
    ...
    private User member() {
        return service.register("Üye", UUID.randomUUID() + "@test.local", "sifre123", null, null,
    ...
    @Test
    void registrationWaitsForMailboxProofAndTokenIsSingleUse() {
        ...
        assertThat(service.confirmStudentEmail(registered, token).isStudent()).isTrue();
        assertThatThrownBy(() -> service.confirmStudentEmail(registered, token)).isInstanceOf(IllegalStateException.class);
    ...
    void failedDeliveryCannotGrantPrivilegesOrBreakRegistration() {
        ...
        rejected(registered, lastToken(school));
    ...
    void dogrulanmamisOkulAdresiGercekSahibiniEngellemez() {
        ...
        User sahibi = service.register("Gerçek Sahip", kurbanAdresi, "sifre123", null, null,
                School.ATATURK_UNIVERSITESI, false, null, null, null);
        assertThat(service.confirmStudentEmail(sahibi, lastToken(kurbanAdresi)).isStudent()).isTrue();
