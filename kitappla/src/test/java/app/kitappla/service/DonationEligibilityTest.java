...
class DonationEligibilityTest {
    ...
    void kampusTeslimindeAdresIstenmez() {
        // Kampüs içi yüz yüze teslimde adres gerekmez (kitappla.features.address=false)
        User donor = user("donor", false, "Adres", null);
    ...
    void filtreSeviyeVeAramayaGoreSuzer() {
        ...
        var ortaokulSonuc = donationService.openDonations(
                new DonationService.Filter(TargetLevel.ORTAOKUL, baslik, true));
        assertThat(ortaokulSonuc).isEmpty();

        var uniSonuc = donationService.openDonations(
                new DonationService.Filter(TargetLevel.UNIVERSITE, baslik, true));
