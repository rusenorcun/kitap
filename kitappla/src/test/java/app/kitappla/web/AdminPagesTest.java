...
class AdminPagesTest {
    ...
    @Test
    void onayliOgrenciBelgelerSayfasindaListelenirVeIptalEdilebilir() throws Exception {
        User yonetici = mk("iptalci", true);
        User ogrenci = mkPending("onayli-ogr", "belge-icerik");
        ogrenci.setStudentStatus(StudentStatus.APPROVED);
        users.save(ogrenci);

        mvc.perform(get("/admin/belgeler").with(user(as(yonetici))))
                .andExpect(status().isOk())
                .andExpect(content().string(containsString("Onaylı öğrenciler")))
                .andExpect(content().string(containsString(ogrenci.getName())));

        mvc.perform(post("/admin/belgeler/" + ogrenci.getId() + "/iptal")
                        .with(user(as(yonetici))).with(csrf()))
                .andExpect(redirectedUrl("/admin/belgeler"))
                .andExpect(flash().attributeExists("basari"));

        User guncel = users.findById(ogrenci.getId()).orElseThrow();
        assertThat(guncel.getStudentStatus()).isEqualTo(StudentStatus.NONE);
        assertThat(guncel.getDocumentPath()).isNull();
    }

    @Test
    void uyelerSayfasindanOgrencilikKaldirilabilir() throws Exception {
        User yonetici = mk("uyeiptal", true);
        User ogrenci = mkPending("uye-ogr", "belge-icerik");
        ogrenci.setStudentStatus(StudentStatus.APPROVED);
        users.save(ogrenci);

        mvc.perform(post("/admin/uyeler/" + ogrenci.getId() + "/ogrenci-kaldir")
                        .with(user(as(yonetici))).with(csrf()))
    ...
    // ---------- Canlı İzleç Testleri ----------

    @Test
    void panoCanliIzlecOzetiniIcerir() throws Exception {
        User yonetici = mk("izlec-pano-admin", true);
        mvc.perform(get("/admin").with(user(as(yonetici))))
                .andExpect(status().isOk())
                .andExpect(content().string(containsString("Canlı Sistem ve Oturum İzleci")))
                .andExpect(content().string(containsString("Aktif Oturum")))
                .andExpect(content().string(containsString("JVM Bellek")));
    }

    @Test
    void yoneticiIzleciGorur() throws Exception {
        User yonetici = mk("izlec-admin", true);
        mvc.perform(get("/admin/izlec").with(user(as(yonetici))))
                .andExpect(status().isOk())
                .andExpect(content().string(containsString("Sistem ve Oturum İzleci")))
                .andExpect(content().string(containsString("Aktif Oturum")))
                .andExpect(content().string(containsString("JVM Bellek (Heap)")))
                .andExpect(content().string(containsString("Açık Oturum Listesi")));
    }

    @Test
    void siradanUyeIzleceErisemez() throws Exception {
        User uye = mk("izlec-uye", false);
        mvc.perform(get("/admin/izlec").with(user(as(uye))))
                .andExpect(status().isForbidden());
        mvc.perform(get("/admin/izlec/canli").with(user(as(uye))))
                .andExpect(status().isForbidden());
    }

    @Test
    void canliIzlecJsonVerisiDoner() throws Exception {
        User yonetici = mk("izlec-json-admin", true);
        mvc.perform(get("/admin/izlec/canli").with(user(as(yonetici))))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.summary.activeSessions").isNumber())
                .andExpect(jsonPath("$.summary.uptimeFormatted").isString())
                .andExpect(jsonPath("$.summary.usedMemoryMb").isNumber())
                .andExpect(jsonPath("$.sessions").isArray());
    }

    @Test
    void apiMonitorUcuCalisir() throws Exception {
        User yonetici = mk("izlec-api-admin", true);
        mvc.perform(get("/api/v1/admin/monitor").with(user(as(yonetici))))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.summary.activeSessions").isNumber())
                .andExpect(jsonPath("$.summary.uptimeFormatted").isString());
    }
    ...
    @Test
    void olmayanOturumSonlandirilincaHataMesajiVerir() throws Exception {
        User yonetici = mk("izlec-expire-admin", true);
        mvc.perform(post("/admin/izlec/oturumlar/gecersiz-session-id/sonlandir")
                        .with(user(as(yonetici))).with(csrf()))
                .andExpect(redirectedUrl("/admin/izlec"))
                .andExpect(flash().attributeExists("hata"));
    }
}
