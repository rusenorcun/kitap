 ...
 */
@SpringBootTest(properties = "kitappla.features.document=true")
...
class BelgeModuTest {
    ...
    void ogrenciBasvurusuBelgeYuklerVeBeklemeyeAlir() throws Exception {
        User u = mk("basvuru");
        var belge = new MockMultipartFile("document", "belge.pdf", "application/pdf", "%PDF-1.4 sahte".getBytes());
    ...
    void basarisizOgrenciBasvurusuBelgeBirakmaz() throws Exception {
        ...
        var belge = new MockMultipartFile("document", "b.pdf", "application/pdf", "%PDF-1.4 x".getBytes());
    ...
    void belgeliKayitOgrenciDogrulamasiniBeklemeyeAlir() throws Exception {
        var belge = new MockMultipartFile("document", "belge.pdf", "application/pdf", "%PDF-1.4 sahte-belge".getBytes());
    ...
    @Test
    void belgeFormuProfilOgrenciSayfasindaGorunur() throws Exception {
        User u = mk("profil-ogr");
        mvc.perform(get("/profil/ogrenci").with(user(as(u))))
                .andExpect(status().isOk())
                .andExpect(content().string(containsString("Öğrenci belgesi ile doğrula")))
                .andExpect(content().string(containsString("Okul e-postası ile anında doğrula")));
    }

    @Test
    void bekleyenBelgeDurumuProfilOgrenciSayfasindaGosterilir() throws Exception {
        User u = mk("bekleyen-ogr");
        u.setStudentStatus(StudentStatus.PENDING);
        u.setDocumentNo("LS-TEST-1234");
        u.setDocumentPath("test.pdf");
        users.save(u);

        mvc.perform(get("/profil/ogrenci").with(user(as(u))))
                .andExpect(status().isOk())
                .andExpect(content().string(containsString("Öğrenci belgeniz incelemede")))
                .andExpect(content().string(containsString("LS-TEST-1234")));
    }
}
