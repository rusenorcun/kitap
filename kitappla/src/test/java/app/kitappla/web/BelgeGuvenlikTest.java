...
class BelgeGuvenlikTest {
    ...
    @Test
    void pdfAdliDuzMetinDosyasiYuklenemez() throws Exception {
        // Sihirli bayt kontrolünü atlatmak için kullanılan kısa önekler (x, sah, tes, ÖRN)
        // artık kabul edilmemeli: .pdf adı tek başına yeterli değil.
        for (String icerik : new String[]{"x", "sahte", "test belgesi", "ÖRNEK ÖĞRENCİ BELGESİ"}) {
            User u = mkUser(false);
            var dosya = new MockMultipartFile("document", "belge.pdf", "application/pdf", icerik.getBytes());

            mvc.perform(multipart("/profil/ogrenci").file(dosya).with(user(new AppUserDetails(u))).with(csrf())
                            .param("schoolLevel", "UNIVERSITE")
                            .param("documentNo", "LS-" + UUID.randomUUID()))
                    .andExpect(redirectedUrl("/profil/ogrenci"))
                    .andExpect(flash().attributeExists("hata"));

            assertThat(users.findById(u.getId()).orElseThrow().getDocumentPath()).isNull();
        }
    }
