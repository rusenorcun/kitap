...
import app.kitappla.api.dto.CreateOfferBody;
import app.kitappla.api.dto.CreateRequestBody;
import app.kitappla.api.dto.CreateSwapBookBody;
import app.kitappla.api.dto.FulfillBody;
...
class RequestAndSwapApiTest {
    ...
    void request_flow_create_and_fulfill() throws Exception {
        ...
        app.kitappla.api.dto.ArrangeMeetingBody reqMeeting = new app.kitappla.api.dto.ArrangeMeetingBody(null, "Saat 15:00", reqMeetingTime);
    ...
    /** Mobil uygulama isteğe kapak ekleyebiliyor (yükleme ucundan dönen yol); kapak kitaba yazılmalı. */
    @Test
    void request_create_keeps_uploaded_cover_and_rejects_unsafe_ones() throws Exception {
        String yerel = "/uploads/covers/" + UUID.randomUUID() + ".jpg";
        mvc.perform(post("/api/v1/requests")
                        .with(csrf())
                        .with(user(as(userA)))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(mapper.writeValueAsString(
                                new CreateRequestBody("Kapaklı İstek " + UUID.randomUUID(), "Yazar K", null, null, yerel))))
                .andExpect(status().isCreated());
        org.assertj.core.api.Assertions.assertThat(books.findAll())
                .anySatisfy(b -> org.assertj.core.api.Assertions.assertThat(b.getCoverUrl()).isEqualTo(yerel));

        for (String kotu : new String[]{"javascript:alert(1)", "/uploads/covers/../../application.properties"}) {
            String baslik = "Güvensiz Kapak " + UUID.randomUUID();
            mvc.perform(post("/api/v1/requests")
                            .with(csrf())
                            .with(user(as(userA)))
                            .contentType(MediaType.APPLICATION_JSON)
                            .content(mapper.writeValueAsString(new CreateRequestBody(baslik, "Yazar G", null, null, kotu))))
                    .andExpect(status().isCreated());
            org.assertj.core.api.Assertions.assertThat(books.findAll())
                    .filteredOn(b -> baslik.equals(b.getTitle()))
                    .singleElement()
                    .satisfies(b -> org.assertj.core.api.Assertions.assertThat(b.getCoverUrl()).isNull());
        }
    }
    ...
    void swap_flow_create_offer_and_accept() throws Exception {
        ...
        app.kitappla.api.dto.ArrangeMeetingBody swapMeeting = new app.kitappla.api.dto.ArrangeMeetingBody(null, "Kütüphane önü", swapMeetingTime);
        ...
        // Both handover
        mvc.perform(post("/api/v1/swaps/" + offerId + "/handover").with(csrf()).with(user(as(userA))))
                .andExpect(status().isNoContent());

        mvc.perform(post("/api/v1/swaps/" + offerId + "/handover").with(csrf()).with(user(as(userB))))
                .andExpect(status().isNoContent());
    }
